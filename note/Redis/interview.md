...
| flushall          | 清空所有库            |
| info              | 获得redis服务器信息   |
| shutdown          | 持久化后关闭redis     |
| config get/set    | 查看/设置配置         |
| redis-server      | redis启动程序         |
| redis-cli         | redis连接程序         |
| redis-benchmark   | redis性能测试程序     |



**如何设置过期时间**

* `expire key seconds` n秒后过期
* `pexpire key miliseconds` n毫秒后过期
* `expire key timstamp`某个时间戳之后
* `pexpire key milisencoudstimstamp` 毫秒级时间戳
...
* 定时删除：在设置过期时间的同时创建一个定时任务，到时间就删除
* 惰性删除：在获得key对应的值时，如果发现过期了才删除
* 定期删除：...
...
* RDB文件紧凑，恢复快，缺点是数据一致性比AOF弱
* AOF支持秒级持久化，但是文件大，恢复慢
...
* 如果数据丢失没有关系，可以不进行任何持久化
* 如果能接收十几分钟的数据丢失，RDB更合适,否则选AOF
* 
...
redis默认开始的rdb，开启AOF需要在配置文件中设置`appendonly yes`
...
* SCAN的参数没有key，因为其迭代的对象是DB内的数据
* 返回值都是数据，第一个值都是下一次迭代的游标
* 时间复杂度：请求O(1),迭代O(n)
* redis2.8及以上版本
...
如果要求强一致性，就不要使用缓存了。只能通过合适的策略来降低不一致的概率
...
redis采用队列将并发访问转为串行访问，多个客户端之间没有竞争关系。
